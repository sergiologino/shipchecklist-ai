...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!screenshot && !pageData.text) return NextResponse.json({ error: "Could not load page content." }, { status: 400 })

    const analysis = await analyzeWithAI(url, screenshot, pageData.text, pageData.headers)

    if (!analysis.categories) return NextResponse.json({ error: "AI could not analyze this page." }, { status: 500 })
    ...
    const categories: CheckCategory[] = analysis.categories.map((cat: any, i: number) => ({
      ...
      checks: (cat.checks || []).map((ch: any) => ({
        ...
        description: ch.description || "", fix: ch.fix || "",
